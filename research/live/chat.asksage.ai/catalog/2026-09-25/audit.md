# Model catalog audit: chat.asksage.ai

Generated 2026-09-25T07:18:28.687Z from public, unauthenticated sources. Listed is not proof of callable: the server may still restrict models per account or organization.
...
| `REACT_APP_has_government_banner` | `true` |
| `REACT_APP_asksage_user_service` | `https://user-server-cac-gov.asksage.ai` |

- Government/CUI-like: **yes**
- Web app's active list: **default** (79 ids)
- Served by get-models: 105 (12 flagged `cui_capable: false`)
...
The instance declares deployment_type="government", classification_level="?". Ask Sage docs: non-CUI models "are not found on all instances... If you are on a CUI-compliant instance, these models will not be available."
...
vars.js sets deployment_type="government" but neither REACT_APP_force_gov_models, REACT_APP_force_dod_models nor REACT_APP_allowed_models, so the web app offers its default list.
...
### [MEDIUM] 34 served model(s) are outside the "gov (reference: deployment is government-like but no list is forced)" allow-list
...
`aws-bedrock-gpt-oss-120b-ts`, `aws-bedrock-gpt-oss-20b-ts`, `aws-bedrock-nemotron-12b-vl-ts`, `aws-bedrock-nemotron-30b-ts`, `aws-bedrock-nemotron-9b-ts`, `aws-bedrock-nemotron-super-3-120b-ts`, `aws-bedrock-titan`, `claude-haiku-4-5-com`, `claude-opus-4-7-com`, `claude-sonnet-4-6-com`, `deepseek-v3.2-com`, `deepseek-v4-flash`, `deepseek-v4-pro`, `google-claude-fable-5`, `google-gemini-3-flash-com`, `google-gemini-3.1-flash-lite-com`, `google-gemini-3.1-pro-com`, `google-gemini-3.5-flash-com`, `google-imagen-3`, `gpt-4.1-gov`, `gpt-4.1-mini-gov`, `gpt-5.4-gov`, `gpt-5.4-sec`, `gpt-5.5`, `gpt-image-2-com`, `gpt-o3`, `gpt-o3-mini-gov`, `gpt-o4-mini`, `groq-70b`, `groq-deepseek`, `groq-llama33`, `groq-llama4-scout`, `kimi-2.6-com`, `kimi-2.7-code-com`
...
### [INFO] 2 id(s) in the web app lists are not served

`gemini-3-6-flash`, `google-gemini-20-flash`
...
- deployment profile: https://chat.asksage.ai/vars.js
- web app bundle: https://chat.asksage.ai/assets/index-DcmfUYyN.js (sha256 1853efb2a2bb2cb5...)
- model catalog: POST https://api.asksage.ai/server/get-models?format=full (API host via chat.* -> api.* (bundle default is https://api.asksage.ai/server))
- fetched: 2026-09-25T07:18:28.687Z
