...
Re-run the audit to refresh the evidence (`phase0/probe/README.md`). `chat.asksage.mil` was not reachable from the environment where this was written. Run the audit from a machine that can reach it.
...
- **§3.1 rates:** `get-models?format=full` serves `token_conversion_rate` (`prompt`, `completion`; a multiplier, Ask Sage tokens per model token) for 95 of 105 models, so the "no API endpoint serves them" line is outdated. Cache, thinking and long-context rates are not served; they existed only in the web app's table. The extension reads the API at runtime and does not maintain a table (PLAN §3.1).
  - **The API and the web app disagree, and billing is not settled.** Compared on 2026-09-25 with a local, uncommitted snapshot of the web app's table (bundle `index-B5zBThzY.js`, fetched 2026-09-22; the live `chat.asksage.ai` bundle is now `index-DcmfUYyN.js`): all 91 models present in both differ. 71 differ by one uniform factor (API multiplier = 0.769 × the table's), the rest by 1.15× to 2.0× in both directions; the API matches the web app's "legacy" second list for only 31 of 85. The single billing measurement (T19, `research/live/test-tenant/probe/2026-09-25-1534-b69656/T19/`) sided with the web app's table: billed 381, table 378, API 291. One model; unresolved until T1–T4/T19 run on more.
  - **Cache rates are not Anthropic-only in that table:** 26 rows, of which 15 Claude, 10 OpenAI (GPT-5.4 and later, at Anthropic's ratios) and one xAI. Gemini has none. See PLAN §2.1 for provenance.
...
- **Run the audit** on every tenant the extension will target, including `.mil` and dedicated instances, before choosing that tenant's defaults.
