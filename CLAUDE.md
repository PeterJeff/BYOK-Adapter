...
- **No Copilot sign-in, ever.** The target machine and the dev machine are never signed in to GitHub Copilot (the login is GitLab). The extension relies on VS Code ≥1.122 letting extension-provided models work with no GitHub account or Copilot plan, so never design or test around a signed-in Copilot account, and never require one.
