...
- [ ] **Phase 0a, folder install on the dev machine** (Command Palette → Developer: Install Extension from Location... → `phase0/smoke-extension`; no packaging): makes E3 real and shows whether `LanguageModelThinkingPart` exists outside the development host. In Agent mode also run `smoke:loop 3 read_file {json}` (several tool rounds in a row, signatures of 1, 8 and 64 KB in the thinking metadata) and read the Signature column and the E4 row. Steps: `phase0/smoke-extension/README.md`.
