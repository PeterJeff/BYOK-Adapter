...
- [ ] **Phase 0a, 5+ tool rounds on the dev machine:** the folder install passed E3 and `smoke:loop 3` (thinking class present, signatures up to 64 KB intact: `research/live/test-tenant/phase0a-e4-loop.md`). Run `smoke:loop 6 read_file {json}` with a real absolute path and save the report. Steps: `phase0/smoke-extension/README.md`.
