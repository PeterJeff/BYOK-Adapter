...
| §2.1 | Cache multipliers (Claude: read 0.1×, 5-min write 1.25×, 1-hour write 2× of the model's own prompt price) are what Ask Sage bills | T1, T15 (ratio to the same model's prompt price, so the §3.1 scale question cancels) | ✅ confirmed on Vertex and Bedrock Claude (Haiku 4.5, Sonnet 4.5, 4.6): a 34.7k write billed 3,105 (1.2505×), its read 253 (0.10×); 1h write 2×, no beta header needed. **Opus 5.5 reads at 0.05×** (measured 2026-09-26, as the table says; 1h write 2×), so the read multiplier is per model; Fable 5.1's 0.025× not measured. `research/live/manual-run-01/probe/2026-09-26-0516-f56b8c/`. `research/live/test-tenant/billing/2026-09-25/measurements.json` |
...
| §3.5 | Billing behavior of a cancelled stream | T9 | ✅ billed for somewhat more output than had arrived, far below the cap (M 43 vs 826 full-cap estimate). `research/live/FINDINGS.md` |
...
| §8 | Which balance (inference or training) embeddings charge | T20 | ✅ training (teach counter +11, inference 0). `research/live/FINDINGS.md` |
...
**Exit: `research/live/FINDINGS.md` (2026-09-26).** Confirms the cache pricing, TTLs, normalization and budget rules; corrects M thinking counts, cancellation billing, the lookback rule and the Gemini default.
...
| 0b | `research/live/FINDINGS.md` with default flavor table, cache policy, normalization rules confirmed/corrected | ✅ 2026-09-26, `research/live/FINDINGS.md`. Gemini's default is open (no agent mode until T18 passes on some id) |
